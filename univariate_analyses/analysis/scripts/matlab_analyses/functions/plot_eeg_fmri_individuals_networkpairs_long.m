function plot_eeg_fmri_individuals_long(allnetworks_allsubs,networks,sublist)
...
%freqs = {'δ'; 'θ'; 'α'; 'β1'; 'β2'; 'γ'};
network_thr ={[-.11 .11];[-.11 .11];[-.11 .11];[-.11 .11]};
...
for i=1:size(allnetworks_allsubs{1},3) % for each subject
    ...
    nexttile
        ...
        set(gca,'xtick',0:5:size(allnetworks_allsubs{j}(:,:,i),2))
    ...
    nexttile
        ...
        print('-opengl','-r600','-dpng',['figs/individual/' sublist{i} '_' networks{j}  '_long.png']); 
    close;
    end
