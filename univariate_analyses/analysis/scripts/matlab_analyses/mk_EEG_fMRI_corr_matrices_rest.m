...
for i = 1:length(subs)
    ...
    mkdir([outdir '/corr_mats/' subs{i} '/' sessions{j}]);
        for k = 1:length(runs)
            for m = 1:length(networks)
                ...
                save([outdir '/corr_mats/' subs{i} '/' sessions{j} '/' runs{k} '_' networks{m} '_rest.mat'],'corr_mat');
                save([outdir '/corr_mats/' subs{i} '/' sessions{j} '/' runs{k} '_' networks{m} '_canonical_rest.mat'],'corr_mat_canonical');
                save([outdir '/corr_mats/' subs{i} '/' sessions{j} '/' runs{k} '_' networks{m} '_canonical_beta_rest.mat'],'corr_mat_canonical_beta');
    ...
    for k = 1:length(networks)
        ...
        save([outdir '/corr_mats/' subs{i} '/' sessions{j} '/' networks{k} '_mean_rest.mat'],'session_mean');
        save([outdir '/corr_mats/' subs{i} '/' sessions{j} '/' networks{k} '_mean_canonical_rest.mat'],'session_mean_canonical');
        save([outdir '/corr_mats/' subs{i} '/' sessions{j} '/' networks{k} '_mean_canonical_beta_rest.mat'],'session_mean_canonical_beta');
    end
    end
    end
end
 ...
 for i=1:length(subs)
     ...
     for j=1:length(networks)
         ...
         for k=1:length(sessions)
           network_corr = load([outdir '/corr_mats/' subs{i} '/' sessions{k} '/' networks{j} '_mean_rest.mat']);
           network_mat(:,:,k) = network_corr.session_mean;
           network_corr_canonical = load([outdir '/corr_mats/' subs{i} '/' sessions{k} '/' networks{j} '_mean_canonical_rest.mat']);
           network_mat_canonical(:,:,k) = network_corr_canonical.session_mean_canonical;
           network_corr_canonical_beta = load([outdir '/corr_mats/' subs{i} '/' sessions{k} '/' networks{j} '_mean_canonical_beta_rest.mat']);
         ...
         save([outdir '/corr_mats/' subs{i} '/' networks{j} '_mean_rest.mat'],'subject_mean');
         save([outdir '/corr_mats/' subs{i} '/' networks{j} '_mean_canonical_rest.mat'],'subject_mean_canonical');
         save([outdir '/corr_mats/' subs{i} '/' networks{j} '_mean_canonical_beta_rest.mat'],'subject_mean_canonical_beta');
     end
     end 
 end
 ...
 for i=1:length(skip_sess1)
     for j=1:length(networks)
        ...
        load([[outdir '/corr_mats/' skip_sess1{i} '/ses-002/' networks{j} '_mean_canonical_rest.mat']]);
        subject_mean_canonical = session_mean_canonical;
        load([[outdir '/corr_mats/' skip_sess1{i} '/ses-002/' networks{j} '_mean_canonical_beta_rest.mat']]);
        subject_mean_canonical_beta = session_mean_canonical_beta;
        save([outdir '/corr_mats/' skip_sess1{i} '/' networks{j} '_mean_rest.mat'],'subject_mean');
        save([outdir '/corr_mats/' skip_sess1{i} '/' networks{j} '_mean_canonical_rest.mat'],'subject_mean_canonical');
        save([outdir '/corr_mats/' skip_sess1{i} '/' networks{j} '_mean_canonical_beta_rest.mat'],'subject_mean_canonical_beta');
     end
 end
 
  for i=1:length(skip_sess2)
     for j=1:length(networks)
        load([[outdir '/corr_mats/' skip_sess2{i} '/ses-001/' networks{j} '_mean_rest.mat']]);
        subject_mean = session_mean;
        load([[outdir '/corr_mats/' skip_sess2{i} '/ses-001/' networks{j} '_mean_canonical_rest.mat']]);
        subject_mean_canonical = session_mean_canonical;
        load([[outdir '/corr_mats/' skip_sess2{i} '/ses-001/' networks{j} '_mean_canonical_beta_rest.mat']]);
        subject_mean_canonical_beta = session_mean_canonical_beta;
        save([outdir '/corr_mats/' skip_sess2{i} '/' networks{j} '_mean_rest.mat'],'subject_mean');
        save([outdir '/corr_mats/' skip_sess2{i} '/' networks{j} '_mean_canonical_rest.mat'],'subject_mean_canonical');
        save([outdir '/corr_mats/' skip_sess2{i} '/' networks{j} '_mean_canonical_beta_rest.mat'],'subject_mean_canonical_beta');
     end
 end
