...
rest = 1; % 0 = use full data; 1 = use rest periods only
...
% Load in ES matrices (runs 002-004) and average within subjects
for i=1:length(networks)
    for j=1:length(sublist)
        es_matrices = []; subject_mean = [];
        for k=1:length(sessions)
            run002 =[]; run003 = []; run004 = []; es_matrix_sess = []; es_matrices =[];
            if rest==1
            run002 = load([data_dir '/' sublist{j} '/' sessions{k} '/run-002_' networks{i} '_canonical_rest.mat']);
            run003 = load([data_dir '/' sublist{j} '/' sessions{k} '/run-003_' networks{i} '_canonical_rest.mat']);
            run004 = load([data_dir '/' sublist{j} '/' sessions{k} '/run-004_' networks{i} '_canonical_rest.mat']);
            else
            run002 = load([data_dir '/' sublist{j} '/' sessions{k} '/run-002_' networks{i} '_canonical.mat']);
            run003 = load([data_dir '/' sublist{j} '/' sessions{k} '/run-003_' networks{i} '_canonical.mat']);
            run004 = load([data_dir '/' sublist{j} '/' sessions{k} '/run-004_' networks{i} '_canonical.mat']);
            end
            run002.corr_mat_canonical(7:end,:) = []; % chop out lags > 10 sec
            run002.corr_mat_canonical = fisherz(run002.corr_mat_canonical); % fisher-z transform
            run003.corr_mat_canonical(7:end,:) = []; % chop out lags > 10 sec
            run003.corr_mat_canonical = fisherz(run003.corr_mat_canonical); % fisher-z transform
            run004.corr_mat_canonical(7:end,:) = []; % chop out lags > 10 sec
            run004.corr_mat_canonical = fisherz(run004.corr_mat_canonical); % fisher-z transform
            es_matrices = cat (3, run002.corr_mat_canonical, run003.corr_mat_canonical, run004.corr_mat_canonical);
            es_matrix_sess(:,:,k) = mean(es_matrices,3);
        end
        subject_mean = mean(es_matrix_sess,3); 
        es_mean{i}(:,:,j) = subject_mean;
    end
end
...
print('-opengl','-r600','-dpng',['figs/es_heatmaps.png']);  
pause; close;
...
print('-opengl','-r600','-dpng',['figs/es_internetwork_heatmaps.png']); 
pause; close;
